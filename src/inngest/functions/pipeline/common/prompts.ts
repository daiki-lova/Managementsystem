...
// V6パイプラインのプロンプト説明（SettingsView表示用）
export const V6_ARTICLE_PROMPT_DESCRIPTION = `
【V6パイプライン概要】
V6は受講生の体験談をベースに、SEO最適化された記事を生成するパイプラインです。

【主な特徴】
1. 体験談の深掘り - 受講生の声から人物像を推測・補完
2. 監修者コメント - 具体的なエピソードと専門家視点を挿入
3. SEO/LLMO最適化 - 構造化データ、FAQ、要約を自動生成
4. 画像生成 - リアルなヨガポーズ写真を生成（WebP形式、80%品質）

【監修者情報の活用】
- 基本情報: 名前、役職、プロフィール
- 指導方針: 理念、スタイル
- 経歴・実績: ヨガ歴、指導歴、指導人数、資格、専門分野
- 表現スタイル: よく使うフレーズ、避ける言葉
- エピソード: 監修者の体験談を記事内で活用

【出力形式】
HTML形式の記事本文 + LLMO用メタデータ（JSON）

※ 詳細なプロンプトはコードベースで管理されています。
