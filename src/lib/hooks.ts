...
export function useAnalyticsOverview(params?: { startDate?: string; endDate?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await analyticsApi.overview(params);
      return response.data!;
    },
...
export function useArticleAnalytics(id: string | null, params?: { startDate?: string; endDate?: string }) {
  return useQuery({
    queryKey: ['analytics', 'article', id, params],
    queryFn: async () => {
      const response = await analyticsApi.article(id!, params);
      return response.data!;
    },
    enabled: !!id,
