...
// スケジュール設定スキーマ（publishAt または scheduledAt を受け付ける）
const scheduleSchema = z.object({
  publishAt: z.string().datetime().optional(),
  scheduledAt: z.string().datetime().optional(),
  version: z.number().optional(), // 楽観的ロック用
}).refine(
  (data) => data.publishAt || data.scheduledAt,
  { message: "publishAt または scheduledAt が必要です" }
);
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return await withAuth(request, async (user) => {
      ...
      const publishAt = new Date(validated.scheduledAt || validated.publishAt!);
